...
int main(int argc, const char *argv[]) {
    ...
    while (1) {
		...
		if (status == IME_DIALOG_RESULT_FINISHED) {
			...
			sprintf(fll, "ux0:data/hhapp/pwd/pwd_%s.hh", userText);
			if (ex(fll)) {
				static char buf[4];
			...
			} else if (!ex("ux0:data/hhapp/init.hh")) {
				sceIoMkdir("ux0:data/hhapp/", 0777);
				sceIoMkdir("ux0:data/hhapp/pwd/", 0777);
				fcp("app0:defset.x", fll);
				fcp("app0:defset.x", "ux0:data/hhapp/pwd/pwdex.hh");
				...
				sceAppMgrLoadExec("app0:eboot.bin", NULL, NULL);
			} else if (!ex("ux0:data/hhapp/pwd/pwdex.hh")) {
				static char buf[4];
				int fd = sceIoOpen("ux0:data/hhapp/init.hh" , SCE_O_RDONLY , 0777);
				sceIoRead(fd, buf, 4);
				sceIoClose(fd);
				int pwdreq = (buf[2] - 0x30);
				if (pwdreq) {
					sceIoMkdir("ux0:data/hhapp/pwd/", 0777);
					fcp("app0:defset.x", "ux0:data/hhapp/pwd/pwdex.hh");
					fcp("app0:defset.x", fll);
